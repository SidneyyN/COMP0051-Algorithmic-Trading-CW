...
Extends 05_pairs_strategy.py with a volatility filter on entry.

Strategy design:
  - Spread:      spread_t = log(BTC_t) - beta * log(ETH_t),  beta = 0.6780
  - Signal:      rolling z-score over 100-bar window
  - Entry:       |z| > 3 AND spread_vol < 1.2 × vol_ref (volatility filter)
  - Exit:        z crosses 0 (full mean reversion) — filter NOT applied to exits
...
Run from project root:
    python notebooks/06_pairs_strategy_vol_filter.py
"""
...
# Volatility filter
VOL_WINDOW     = 100            # rolling window for spread volatility (bars)
...
# ============================================================
# Step 3b — Volatility Filter
# ============================================================

def compute_vol_filter(df: pd.DataFrame,
                       vol_window:     int   = VOL_WINDOW,
                       vol_ref_window: int   = VOL_REF_WINDOW,
                       vol_mult:       float = VOL_MULT) -> pd.DataFrame:
    """
    Compute a boolean allow_trade column that is True only when the
    current spread volatility is within the 'normal' regime:

        spread_vol = rolling std of spread.diff()  over vol_window bars
        vol_ref    = rolling median of spread_vol  over vol_ref_window bars
        allow_trade = spread_vol < vol_mult * vol_ref

    Applied to entry only — exits are never blocked by this filter.
    """
    df = df.copy()
    spread_diff       = df["spread"].diff()
    df["spread_vol"]  = spread_diff.rolling(vol_window,     min_periods=vol_window).std()
    df["vol_ref"]     = df["spread_vol"].rolling(vol_ref_window, min_periods=vol_ref_window).median()
...
def generate_positions(df: pd.DataFrame,
    ...
    Controls:
        ...
        allow_trade — boolean column; gates entry only (exits always proceed)
    """
    n           = len(df)
    z           = df["zscore"].values
    allow_trade = df["allow_trade"].values   # volatility filter (entry gate)
    ...
    for i in range(1, n):
        ...
        allowed = allow_trade[i - 1] # lagged filter flag (no lookahead)
        ...
        # volatility filter gates entry only; exits are never blocked
        if state == 0:
            if cooldown_left > 0:
                cooldown_left -= 1
            elif allowed:                    # <-- volatility filter applied here
                if zi < -entry_z:
                    state      = 1
                    hold_count = 0
                elif zi > entry_z:
...
def plot_spread_zscore(df: pd.DataFrame, is_end: str):
    ...
    # shade filtered-out bars
    ax1.fill_between(df.index, ax1.get_ylim()[0], ax1.get_ylim()[1],
                     where=~df["allow_trade"].fillna(False),
                     color="grey", alpha=0.15, label="Vol filter (blocked)")
    ax1.axvline(pd.Timestamp(is_end, tz="UTC"), color="black",
                linestyle="--", lw=1.2, label="IS/OOS split")
    ax1.set_ylabel("Spread (log units)")
    ax1.set_title("BTC–ETH Spread with Rolling Bands (grey = entry blocked by vol filter)")
    ...
    # --- Panel 3: spread volatility vs reference ---
    ax3 = axes[2]
    ax3.plot(df.index, df["spread_vol"], lw=0.7, color="darkorange", label="Spread vol")
    ax3.plot(df.index, df["vol_ref"] * VOL_MULT, lw=1.0, color="red",
             linestyle="--", label=f"{VOL_MULT}× vol_ref (threshold)")
    ax3.fill_between(df.index, 0, df["spread_vol"],
                     where=~df["allow_trade"].fillna(False),
                     color="red", alpha=0.2, label="Filtered (vol too high)")
    ax3.axvline(pd.Timestamp(is_end, tz="UTC"), color="black",
                linestyle="--", lw=1.2)
    ax3.set_ylabel("Spread volatility")
    ax3.set_title("Spread Volatility vs Filter Threshold")
    ax3.legend(fontsize=8)

    ax3.xaxis.set_major_formatter(mdates.DateFormatter("%b %Y"))
    fig.autofmt_xdate()
    plt.tight_layout()
    path = os.path.join(FIGURES_DIR, "vol_filter_spread_zscore.png")
...
def plot_equity_curve(df: pd.DataFrame, is_end: str):
    ...
    fig.autofmt_xdate()
    plt.tight_layout()
    path = os.path.join(FIGURES_DIR, "vol_filter_equity_curve.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"  Saved: {path}")
...
def plot_positions(df: pd.DataFrame, is_end: str):
    ...
    fig.autofmt_xdate()
    plt.tight_layout()
    path = os.path.join(FIGURES_DIR, "vol_filter_positions.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"  Saved: {path}")
...
def plot_cost_sensitivity(df_base: pd.DataFrame, cost_range_bps: list):
    ...
    plt.tight_layout()
    path = os.path.join(FIGURES_DIR, "vol_filter_cost_sensitivity.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"  Saved: {path}")
    return res_df
...
def save_outputs(df: pd.DataFrame, trades: pd.DataFrame,
    ...
    cols_out = ["spread", "spread_mean", "spread_std", "zscore",
                "spread_vol", "vol_ref", "allow_trade",
                "position", "gross_pnl", "cost", "net_pnl"]
    path_bt = os.path.join(TABLES_DIR, "vol_filter_backtest.csv")
    df[cols_out].to_csv(path_bt)
    print(f"  Saved: {path_bt}")

    # Trade log
    path_tl = os.path.join(TABLES_DIR, "vol_filter_trade_log.csv")
    trades.to_csv(path_tl, index=False)
    print(f"  Saved: {path_tl}")

    # Performance metrics
    path_pm = os.path.join(TABLES_DIR, "vol_filter_performance.csv")
    pd.DataFrame(metrics_list).to_csv(path_pm, index=False)
    print(f"  Saved: {path_pm}")

    # Cost sensitivity
    path_cs = os.path.join(TABLES_DIR, "vol_filter_cost_sensitivity.csv")
...
def main():
    print("=" * 60)
    print("Pairs Trading Strategy — BTC/ETH (Volatility Filter)")
    print("=" * 60)

    # --- Load ---
    df = load_data(DATA_PATH)

    # --- Compute spread, z-score, and volatility filter ---
    df = compute_spread(df, beta=BETA)
    df = compute_zscore(df, window=ROLL_WINDOW)
    df = compute_vol_filter(df)
